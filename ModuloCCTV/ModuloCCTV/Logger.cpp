...
const string logFileName = "log\\bitacora.log";
...
string Logger::getCurrentTime()
{
	...
	{
		cerr << err
			<< ": Invalid argument to _localtime64_s. \n";
		return currTime;
	}

	// Convert to an ASCII representation. 
	err = asctime_s(timebuf, 32, &newtime);
	if (err)
	{
		cerr << err
			<< ": Invalid argument to asctime_s. \n";
		return currTime;
	}

	currTime.assign(timebuf);
	currTime.pop_back(); // remove newline char @ end of string
	return currTime;
}
