...
CAntialiasingWu::CAntialiasingWu()
{
}
...
void CAntialiasingWu::DrawWuLine(CDC *pDC, short X0, short Y0, short X1, short Y1, 
...
void CAntialiasingWu::DrawPixel(CDC *pDC, short x, short y, short color)
{
	pDC->SetPixel(x, y, RGB(color, color, color));
}

void CAntialiasingWu::DrawWuCirlce(CDC * pDC, int x, int y, int r, COLORREF clrLine)
{
	short x1, y1, x2, y2;
	float dt = 5;
	...
	for (float theta = 0; theta< 360; theta += dt)
	{
		...
		DrawWuLine(pDC, x1, y1, x2, y2, clrLine);
	}
}

void CAntialiasingWu::DrawWuLine(CDC *pDC, int X0, int Y0, int X1, int Y1, COLORREF clrLine)
{
	/* Make sure the line runs top to bottom */
	if (Y0 > Y1)
	{
