...
#ifdef _WIN64
typedef   unsigned __int64      IVS_ULONG;
#else
...
#define IVS_EVENT_IA_PUSH_DRAW_CALLBACK                 10105   // 智能分析绘图回调推送
...
#define IVS_EVENT_REPORT_USER_INFOCHANGE				10310	//摄像机和用户变更通知
...
#define IVS_CAMERACODE_LEN	20	//摄像机ID，20位数字
#define MAX_CHANNELID_VALUE 255	// max support key-value of  nvr code & cameracode
...
typedef struct
{
	...
	IVS_CHAR pPWD[IVS_PWD_LEN];                     // 密码，登录类型为0（基本账号）时，不能为空
...
typedef struct
{
	...
	IVS_UINT32 uiLoginType;							// 登录类型：参考 IVS_LOGIN_TYPE 0–基本身份验证,	1–Windows域身份验证—当前用户，2–Windows域身份验证—其他用户, 3-SSO类型登录;
	IVS_CHAR cDomainName[IVS_DOMAIN_LEN];			// windows域名;
	IVS_CHAR cMachineName[IVS_MACHINE_NAME_LEN];	// PC机器名;
...
typedef struct
{
    IVS_CHAR cTokenID[IVS_TOKEN_LEN];           // TokenID
    IVS_CHAR cUserName[IVS_NAME_LEN];           // 用户名
    IVS_CHAR pPWD[IVS_PWD_LEN];                 // 用户密码
    IVS_IP  stClientIP;                         // 客户端所在机器IP
    IVS_CHAR cReserve[32];                      //保留字段
}IVS_VALIDATE_USER_INFO;

// 用户登录信息:增加TokenID，和第三方客户端IP和端口
typedef struct
{
    IVS_CHAR cTokenID[IVS_TOKEN_LEN];           // TokenID
    IVS_CHAR cUserName[IVS_NAME_LEN];		        // 用户名
    ...
    IVS_UINT32 uiClientType;						// 客户端类型：参考 IVS_CLIENT_TYPE 0-PC客户端, 1-web浏览器, 2-移动客户端, 3-PC客户端子进程，4-电视墙进程
    IVS_CHAR cReserve[32];                          //保留字段
}IVS_LOGIN_INFO_BYTOKEN;

// 用户密码修改信息
typedef struct
{
    IVS_CHAR cTokenID[IVS_TOKEN_LEN];           // TokenID
    IVS_CHAR pOldPWD[IVS_PWD_LEN];              // 旧用户密码
    IVS_CHAR pNewPWD[IVS_PWD_LEN];              // 新用户密码
    IVS_IP  stClientIP;                         // 客户端所在机器IP
    IVS_CHAR cUserName[IVS_NAME_LEN];		        // 用户名
    IVS_CHAR cReserve[32];                      //保留字段
}IVS_CHANGE_USERPWD_INFO;
...
typedef enum
{
    ...
    //车辆元数据相关
    QUERY_VEHICLE_SNAP_PLACE = 2000,    //车辆抓拍地点，值为摄像头名称
    QUERY_VEHICLE_DERICTION ,	        //车辆方向，参考ITS_DIRECTION_TYPE_E
    QUERY_VEHICLE_CUSTOM_ILLEGATYPE ,   //违章类型，暂未开放，不能使用
    QUERY_VEHICLE_VEHICLE_TYPE ,	    //车辆类型，参考ITS_VEHICLE_TYPE
    QUERY_VEHICLE_VEHICLE_COLOR ,		//车辆颜色，参考ITS_VEHICLE_COLOR
    QUERY_VEHICLE_LANE_CHANNEL ,		//车道编号
    QUERY_VEHICLE_PLATE_COLOR ,			//车牌颜色，参考ITS_PLATE_COLOR
    QUERY_VEHICLE_PLATE_TYPE ,			//车牌类型，参考IVS_ITS_PLATE_TYPE
    ...
    QUERY_VEHICLE_START_TIME ,			//查询开始时间，格式为“YYYYMMDDHHMMSS”
    QUERY_VEHICLE_END_TIME,				//查询结束时间，格式为“YYYYMMDDHHMMSSmmm”
    QUERY_VEHICLE_PIC_ATTR ,			//照片属性，标记卡口图片或备份图片，0是全部、1是卡口、2是备份，参考ITS_PIC_ATTRIBUTE
...
typedef struct
{
    ...
    IVS_CHAR    cNVRCode[IVS_NVR_CODE_LEN];
	IVS_CHAR    cReserve[32];                  // 保留字段
}IVS_DOWNLOAD_PARAM;
...
typedef enum
{
	//modify ，去掉“智能分析录像	”。Date：20140520
...
typedef enum
{
    ...
    DEVICE_TYPE_CAMERA_AND_SHADOW = 32,		//摄像头设备和影子摄像头

	DEVICE_TYPE_CAMERA_EX,					// 摄像头设备，带有高度和经纬度信息
}IVS_DEVICE_TYPE;
...
typedef enum
{
    RELIABLE_TRANSMISSION_NONE,			 //关闭
    RELIABLE_TRANSMISSION_FEC,					//FEC 前向纠错
    RELIABLE_TRANSMISSION_ARQ,				//ARQ自动重传请求,暂不支持
    RELIABLE_TRANSMISSION_HARQ,			//HARQ 混合自动重传请求，暂不支持
	RELIABLE_TRANSMISSION_SEC,					//SEC 超级前向纠错
...
typedef enum
{
	...
	CONFIG_CAMERA_SEC_CFG,                  // 摄像机SEC参数 IVS_CAMERA_SEC_CFG
}IVS_CONFIG_TYPE;
...
typedef struct
{
	...
	IVS_CHAR cLongitude[IVS_LONGITUDE_LATITUDE_LEN];					//经度
	IVS_CHAR cLatitude[IVS_LONGITUDE_LATITUDE_LEN];						//纬度
	IVS_FLOAT fHeight;								//摄像机安装高度

	IVS_CHAR	cReserve[32];
}IVS_CAMERA_BRIEF_INFO_EX;
...
typedef struct
{
	IVS_UINT32   uiTotal;												// 总记录数
	IVS_INDEX_RANGE stIndexRange;						// 分页信息
	IVS_CAMERA_BRIEF_INFO_EX stCameraBriefInfo[1];
}IVS_CAMERA_BRIEF_INFO_LIST_EX;
...
typedef struct
{
	IVS_UINT32   uiTotal;												// 总记录数
	IVS_INDEX_RANGE stIndexRange;						// 分页信息
	IVS_SHADOW_CAMERA_BRIEF_INFO stCameraBriefInfo[1];
}IVS_SHADOW_CAMERA_BRIEF_INFO_LIST;
...
typedef struct  
{ 
    IVS_UINT32  uiNotifyType;              
    IVS_UINT32  uiOperType;
    IVS_CHAR  cDomainCode[IVS_DOMAIN_CODE_LEN];          
    IVS_CHAR    cCodeID[IVS_DEV_CODE_LEN];
    IVS_USER_INFO cUserDetail;
    IVS_CAMERA_BRIEF_INFO_ALL  cCameraDetail;
    IVS_USER_GROUP cUserGroupDetail;
    IVS_DEVICE_GROUP cCameraGroupDetail;

}IVS_USER_CAMERA_INFO_CHANGE;

typedef struct  
{ 
    IVS_UINT32   uiTotal;                                   // 总记录数
    IVS_USER_CAMERA_INFO_CHANGE pstUserCameraInfoChange[1];
}IVS_USER_CAMERA_INFO_CHANGE_LIST;


typedef enum 
{
    ADD_USER_CAMERA_GROUP  = 1,
    DELET_USER_CAMERA_GROUP  = 2,
    MODIFY_USER_CAMERA_GROUP  = 3,
    ADD_CAMERA_TO_GROUP  = 4,
    DELETE_CAMERA_FROM_GROUP = 5
}USER_CAMERA_INFO_CHANGE_OPER_TYPE;
...
typedef enum 
{
    ...
    USER_GROUP_INFO  = 3,
    CAMERA_GROUP_INFO  = 4
}USER_CAMERA_INFO_CHANGE_NOTIFY_TYPE;
...
typedef struct
{
	IVS_UINT32   uiTotal;               // 系统实际域路由个数
	//IVS_INDEX_RANGE stIndexRange;     // 分页信息
	IVS_DOMAIN_ROUTE_EX stDomainRoute[1];  // 域路由信息
}IVS_DOMAIN_ROUTE_LIST_EX;
...
typedef struct
{
	IVS_SERVICE_TYPE ServiceType;								    // 含义和取值见枚举IVS_SERVICE_TYPE定义
	IVS_PACK_PROTOCOL_TYPE PackProtocolType;    // 含义和取值见枚举IVS_PACK_PROTOCOL_TYPE定义。场景：比如机顶盒支持TS流，则解包需要解TS头；PS用于国标摄像机；一般ES比较多，为兼容性，应该开放出来。
	IVS_TIME_SPAN  stTimeSpan;						  // 录像时间段需要，可选填。如果是回放、下载、平台或前端录像需要。年月日T时分秒Z-年月日T时分秒Z（20160214T155505Z-20160214T160505Z）。一般通过查询录像获取到，和eSDK返回的录像时间段统一。
	char szFileName[FILE_NAME_LEN];                          // 选填参数。前端录像回放、下载时用。参数是通过查询前端录像的时候，前端返回的文件名。
	IVS_INT32 iClientType;													 // 可选参数，客户端类型，与szEncryAlgorithm配合标识视频是否需要加密， 0-IVS CU或eSDK，1-标准RTSP客户端。内部默认值1，可选填0，用于后续扩展
	IVS_STREAM_TYPE         StreamType;						// 码流类型,1-主码流 2-辅码流1 3-辅码流2，默认为1
	IVS_PROTOCOL_TYPE       ProtocolType;                  // 协议类型，1-UDP 2-TCP，默认为1
	IVS_MEDIA_TRANS_MODE    TransMode;				// 是否直连优先,0-否 1-是，默认为0
	IVS_BROADCAST_TYPE      BroadCastType;				// 是否支持组播，0-不支持 1-支持，默认为0 
	IVS_VIDEO_DEC_TYPE             VideoDecType;		// 解码端支持的视频编码格式，参考枚举 IVS_VIDEO_DEC_TYPE
	IVS_AUDIO_DEC_TYPE      AudioDecType;				// 解码端支持的音频编码格式，参考枚举IVS_AUDIO_DEC_TYPE
	IVS_CHAR	cNVRCode[IVS_NVR_CODE_LEN];
	IVS_CHAR    cReserve[32];
}IVS_URL_MEDIA_PARAM;
...
typedef struct
{
    ...
    IVS_CHAR        cNVRCode[IVS_NVR_CODE_LEN];   // 要回放的录像文件所在的NVR的NVRCode
}IVS_PLAYBACK_PARAM;
...
typedef struct
{
	...
	IVS_CHAR cNVRCode[IVS_NVR_CODE_LEN];	// 设备所属NVR编码, 表示该设备被该NVR管理
	IVS_CHAR cDevProtocolType [IVS_MAX_PROTOCOLTYPE_NAME_LEN];		// 主设备协议类型: "TEYES"-千里眼   "ONVIF"  "HWSDK"-华为SDK "HIKSDK"-海康SDK
	IVS_CHAR cDevGroupID [IVS_DEVICE_GROUP_LEN];           //设备组编码，如果为空的话，默认添加到根组
	IVS_CHAR cOrigDevCode[IVS_DEVICE_NAME_LEN];	//设备原始编码
}IVS_SHADOW_IPC_INFO;
...
{
	IVS_UINT32	uiNum;
	IVS_DEVICE_CODE	stCodeList[1];
} IVS_SHADOW_IPC_LIST;

// 摄像机SEC参数
typedef struct
{
	IVS_UINT32  uiReliableTransmission;     // 首个可靠传输类型，值参考 IVS_RELIABLE_TRANSMISSION_TYPE
	IVS_UINT32  uiDataDropPercent;           // 首个可靠传输类型下的丢包率，SEC模式下，丢包率    1:  4%丢包率   2:  8%丢包率  3:  12%  丢包率   4:  16%  丢包率   5:  20%丢包率
																				// FEC模式下: 丢包率    1:  1%丢包率   2:  2%丢包率  3:  3%  丢包率   4:  4%  丢包率   5:  5%丢包率
	IVS_UINT32  uiSelReliableTransmission[IVS_RELIABLE_TRANSMISSION_TYPE_NUM];	//可选择的传输类型：NONE FEC ARQ HARQ SEC
...
typedef struct
{
	IVS_UINT32 uiChannel;    //NVR通道号
}IVS_CHANNEL;

//摄像机状态
typedef struct
{
	IVS_BOOL	bStatus;																			 //是否在线,0-离线,1-在线
	IVS_CHAR	cCameraCode[IVS_DEV_CODE_LEN];					//摄像机ID,20位有效数字
	IVS_CHAR	cReserve[64];															//预留
}IVS_CAMERACODE_INFO;
...
/******************************************************************
 function   :	DisplayCallBackFrame
 description:	获取解码后码流回调函数(录像回放)
 input      :	nType 画面格式,参考IVS_PICTURE_FORMAT
				pYBuf YUV数据的Y数据分量
				pUBuf YUV数据的U数据分量
				pVBuf YUV数据的V数据分量
				nWidth 画面的宽度
 ...
 return     :	IVS_VOID
*******************************************************************/
typedef IVS_VOID (__SDK_CALL * DisplayCallBackFrame)(IVS_UINT32 nType, IVS_UCHAR *pYBuf,
...
typedef struct
{
    ...
    IVS_CHAR        cClusterCode[IVS_CLUSTER_CODE_LEN + 1];	// 集群编码
