...
extern "C"
{
	...
	* return		: 成功返回0（IVS_SUCCEED），失败返回错误码
	* remark		: pRspXml使用完后，内存需要调用IVS_SDK_ReleaseBuffer接口回收
	*************************************************************************/
	...
	* description: 查询告警联动策略（安全函数）
	* input      : iSessionID 登录成功后的会话ID
	* input      : pReqXml 查询条件
	* output     : pRspXml 查询到的告警联动策略
	* return     : 成功返回0，失败返回错误码
	* remark     : pRspXml使用完后，内存需要调用IVS_SDK_ReleaseBuffer接口回收
	*************************************************************************/
	...
	* output		: pDeviceList  设备列表指针，具体说明如下：
                                摄像头设备：IVS_CAMERA_BRIEF_INFO_LIST
								告警设备：IVS_ALARM_BRIEF_INFO_LIST
                                外域告警设备：IVS_ALARM_BRIEF_INFO_LIST
                                影子摄像头：IVS_SHADOW_CAMERA_BRIEF_INFO_LIST
								摄像头设备和影子摄像头：IVS_SHADOW_CAMERA_BRIEF_INFO_LIST
								摄像头设备，带有高度和经纬度信息：IVS_CAMERA_BRIEF_INFO_LIST_EX
	* return		: 成功返回0（IVS_SUCCEED），失败返回错误码
	* remark		: NA
	*******************************************************************/
  ...
  *******************************************************************/
  ...
  /******************************************************************
    ...
    * description	: 查询告警联动动作详情（安全函数）
    * input			: iSessionID 登录成功后的会话ID
	* input			: pReqXml 查询条件
	* output		: pRsqXml 查询到的告警联动动作详情
    * return		: 成功返回IVS_SUCCESSED，失败返回错误码
	* remark		: pRspXml使用完后，内存需要调用IVS_SDK_ReleaseBuffer接口回收
	* history		: 2017-1-9
    **************************************************************************/
  ...
  *******************************************************************/
   ...
   * description: 查询过车图片列表
   * input		: iSessionID 登录成功后的会话ID
   * input		: pUnifiedQuery 查询信息
   * input		: uiBufferSize 存放查询到的图片内存大小
   * output 	: pVehicleList 存放查询到的图片内存指针
   * return 	: 成功返回0，失败返回错误码
   * remark 	: NA
   *************************************************************************/
   ...
   * description	: 开始上传图片,获取上传句柄
   * input			: iSessionID 登录成功后的会话ID
   * input/output	: pStoreNvrInfo 图片平台存储信息
						[in]pStoreNvrInfo->cDomainCode 域编码
						[in]pStoreNvrInfo->cClusterCode 集群编码（集群模式下必填，堆叠模式下为空）
						[in/out]pStoreNvrInfo->cNVRCode NVR编码;（堆叠模式下，cNVRCode为入参；集群模式下，cNVRCode为出参）
   * output 		: pHandle 上传句柄
   * return 		: 成功返回0，失败返回错误码
   * remark 		: 目前未开放获取集群编码的接口，需要从集群配置页面上获取。
   *************************************************************************/
   ...
   * description	: 通过句柄上传图片
   * input			: iSessionID 登录成功后的会话ID; 
   * input			: pHandle 上传图片的句柄;
   * input/output	: pImgInfo[] 上传图片的结构体指针数组;
   * input			: uiPicNum 上传的图片个数
   * output 		: NA
   * return 		: 成功返回0，失败返回错误码
   * remark 		: NA
   *************************************************************************/
   ...
   * description	: 下载图片
   * input			: iSessionID 登录成功后的会话ID
   * input			: pStoreNvrInfo 图片平台存储信息
   * input/output	: pImgInfo[] 需要下载的图片信息和下载后的图片数据
   * input			: uiPicNum 下载图片个数
   * output 		: NA
   * return 		: 成功返回0，失败返回错误码
   * remark 		: pStoreNvrInfo只需入参cDomainCode和cNVRCode。集群模式下入参的cNVRCode为开始上传时的出参。
   *************************************************************************/
   ...
   * description	: 获取单点登录的ticket
   * input			: iSessionID 登录成功后的会话ID
   * input			: uiBufferSize 存储Ticket的内存字节长度（不小于64字节）
   * output			: pTicket 一次性认证凭据
   * return 		: 成功返回0，失败返回错误码
   * remark 		: Ticket为一次性认证凭据，有效时间为5分钟
   *************************************************************************/
	...
	* name       : IVS_SDK_Login
	* description: 用户登录
	* input      : IVS_LOGIN_INFO_BYTOKEN 登陆入参结构体
                       （ cTokenID TokenID
                       pUserName 用户名
	                    pPassword 密码
	                    pIP       服务器IP地址
	                    iPort     服务器端口
                        stClientIP 登录第三方客户端IP
	...
	**************************************************************************/
    ...
    description: 修改用户密码
    input : IVS_INT32 iSessionID            登录成功后的会话ID
    input : const IVS_CHANGE_USERPWD_INFO* pUserInfo    待修改的用户信息
    return : 成功返回0，失败返回错误码
    *******************************************************************/
    ...
    *******************************************************************/
    ...
    description: 获取用户名和密码检验token
    input : IVS_INT32 iSessionID            登录成功后的会话ID
	input			: uiBufferSize 存储Token的内存字节长度（不小于64字节）
    output : IVS_CHAR * pTokenID  返回的Token，最大长度为IVS_TOKEN_LEN	
    return : 成功返回0，失败返回错误码
    *******************************************************************/
    ...
    description: 设置播放速度的类型
    input : IVS_INT32 iSessionID            登录成功后的会话ID
	input			: 0表示真实速度，1表示默认速度。
    return : 成功返回0，失败返回错误码
    *******************************************************************/
